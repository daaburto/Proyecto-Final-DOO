package org.pokemon;

import javax.swing.*;

public enum Species {
    BULBASAUR(Generation1.BULBASAUR),
    IVYSAUR(Generation1.IVYSAUR),
    VENUSAUR(Generation1.VENUSAUR),
    CHARMANDER(Generation1.CHARMANDER),
    CHARMELEON(Generation1.CHARMELEON),
    CHARIZARD(Generation1.CHARIZARD),
    SQUIRTLE(Generation1.SQUIRTLE),
    WARTORTLE(Generation1.WARTORTLE),
    BLASTOISE(Generation1.BLASTOISE),
    CATERPIE(Generation1.CATERPIE),
    METAPOD(Generation1.METAPOD),
    BUTTERFREE(Generation1.BUTTERFREE),
    WEEDLE(Generation1.WEEDLE),
    KAKUNA(Generation1.KAKUNA),
    BEEDRILL(Generation1.BEEDRILL),
    PIDGEY(Generation1.PIDGEY),
    PIDGEOTTO(Generation1.PIDGEOTTO),
    PIDGEOT(Generation1.PIDGEOT),
    RATTATA(Generation1.RATTATA),
    RATICATE(Generation1.RATICATE),
    SPEAROW(Generation1.SPEAROW),
    FEAROW(Generation1.FEAROW),
    EKANS(Generation1.EKANS),
    ARBOK(Generation1.ARBOK),
    PIKACHU(Generation1.PIKACHU),
    RAICHU(Generation1.RAICHU),
    SANDSHREW(Generation1.SANDSHREW),
    SANDSLASH(Generation1.SANDSLASH),
    NIDORAN_FEMALE(Generation1.NIDORAN_FEMALE),
    NIDORINA(Generation1.NIDORINA),
    NIDOQUEEN(Generation1.NIDOQUEEN),
    NIDORAN_MALE(Generation1.NIDORAN_MALE),
    NIDORINO(Generation1.NIDORINO),
    NIDOKING(Generation1.NIDOKING),
    CLEFAIRY(Generation1.CLEFAIRY),
    CLEFABLE(Generation1.CLEFABLE),
    VULPIX(Generation1.VULPIX),
    NINETALES(Generation1.NINETALES),
    JIGGLYPUFF(Generation1.JIGGLYPUFF),
    WIGGLYTUFF(Generation1.WIGGLYTUFF),
    ZUBAT(Generation1.ZUBAT),
    GOLBAT(Generation1.GOLBAT),
    ODDISH(Generation1.ODDISH),
    GLOOM(Generation1.GLOOM),
    VILEPLUME(Generation1.VILEPLUME),
    PARAS(Generation1.PARAS),
    PARASECT(Generation1.PARASECT),
    VENONAT(Generation1.VENONAT),
    VENOMOTH(Generation1.VENOMOTH),
    DIGLETT(Generation1.DIGLETT),
    DUGTRIO(Generation1.DUGTRIO),
    MEOWTH(Generation1.MEOWTH),
    PERSIAN(Generation1.PERSIAN),
    PSYDUCK(Generation1.PSYDUCK),
    GOLDUCK(Generation1.GOLDUCK),
    MANKEY(Generation1.MANKEY),
    PRIMEAPE(Generation1.PRIMEAPE),
    GROWLITHE(Generation1.GROWLITHE),
    ARCANINE(Generation1.ARCANINE),
    POLIWAG(Generation1.POLIWAG),
    POLIWHIRL(Generation1.POLIWHIRL),
    POLIWRATH(Generation1.POLIWRATH),
    ABRA(Generation1.ABRA),
    KADABRA(Generation1.KADABRA),
    ALAKAZAM(Generation1.ALAKAZAM),
    MACHOP(Generation1.MACHOP),
    MACHOKE(Generation1.MACHOKE),
    MACHAMP(Generation1.MACHAMP),
    BELLSPROUT(Generation1.BELLSPROUT),
    WEEPINBELL(Generation1.WEEPINBELL),
    VICTREEBEL(Generation1.VICTREEBEL),
    TENTACOOL(Generation1.TENTACOOL),
    TENTACRUEL(Generation1.TENTACRUEL),
    GEODUDE(Generation1.GEODUDE),
    GRAVELER(Generation1.GRAVELER),
    GOLEM(Generation1.GOLEM),
    PONYTA(Generation1.PONYTA),
    RAPIDASH(Generation1.RAPIDASH),
    SLOWPOKE(Generation1.SLOWPOKE),
    SLOWBRO(Generation1.SLOWBRO),
    MAGNEMITE(Generation1.MAGNEMITE),
    MAGNETON(Generation1.MAGNETON),
    FARFETCH_D(Generation1.FARFETCH_D),
    DODUO(Generation1.DODUO),
    DODRIO(Generation1.DODRIO),
    SEEL(Generation1.SEEL),
    DEWGONG(Generation1.DEWGONG),
    GRIMER(Generation1.GRIMER),
    MUK(Generation1.MUK),
    SHELLDER(Generation1.SHELLDER),
    CLOYSTER(Generation1.CLOYSTER),
    GASTLY(Generation1.GASTLY),
    HAUNTER(Generation1.HAUNTER),
    GENGAR(Generation1.GENGAR),
    ONIX(Generation1.ONIX),
    DROWZEE(Generation1.DROWZEE),
    HYPNO(Generation1.HYPNO),
    KRABBY(Generation1.KRABBY),
    KINGLER(Generation1.KINGLER),
    VOLTORB(Generation1.VOLTORB),
    ELECTRODE(Generation1.ELECTRODE),
    EXEGGCUTE(Generation1.EXEGGCUTE),
    EXEGGUTOR(Generation1.EXEGGUTOR),
    CUBONE(Generation1.CUBONE),
    MAROWAK(Generation1.MAROWAK),
    HITMONLEE(Generation1.HITMONLEE),
    HITMONCHAN(Generation1.HITMONCHAN),
    LICKITUNG(Generation1.LICKITUNG),
    KOFFING(Generation1.KOFFING),
    WEEZING(Generation1.WEEZING),
    RHYHORN(Generation1.RHYHORN),
    RHYDON(Generation1.RHYDON),
    CHANSEY(Generation1.CHANSEY),
    TANGELA(Generation1.TANGELA),
    KANGASKHAN(Generation1.KANGASKHAN),
    HORSEA(Generation1.HORSEA),
    SEADRA(Generation1.SEADRA),
    GOLDEEN(Generation1.GOLDEEN),
    SEAKING(Generation1.SEAKING),
    STARYU(Generation1.STARYU),
    STARMIE(Generation1.STARMIE),
    MR_MIME(Generation1.MR_MIME),
    SCYTHER(Generation1.SCYTHER),
    JYNX(Generation1.JYNX),
    ELECTABUZZ(Generation1.ELECTABUZZ),
    MAGMAR(Generation1.MAGMAR),
    PINSIR(Generation1.PINSIR),
    TAUROS(Generation1.TAUROS),
    MAGIKARP(Generation1.MAGIKARP),
    GYARADOS(Generation1.GYARADOS),
    LAPRAS(Generation1.LAPRAS),
    DITTO(Generation1.DITTO),
    EEVEE(Generation1.EEVEE),
    VAPOREON(Generation1.VAPOREON),
    JOLTEON(Generation1.JOLTEON),
    FLAREON(Generation1.FLAREON),
    PORYGON(Generation1.PORYGON),
    OMANYTE(Generation1.OMANYTE),
    OMASTAR(Generation1.OMASTAR),
    KABUTO(Generation1.KABUTO),
    KABUTOPS(Generation1.KABUTOPS),
    AERODACTYL(Generation1.AERODACTYL),
    SNORLAX(Generation1.SNORLAX),
    ARTICUNO(Generation1.ARTICUNO),
    ZAPDOS(Generation1.ZAPDOS),
    MOLTRES(Generation1.MOLTRES),
    DRATINI(Generation1.DRATINI),
    DRAGONAIR(Generation1.DRAGONAIR),
    DRAGONITE(Generation1.DRAGONITE),
    MEWTWO(Generation1.MEWTWO),
    MEW(Generation1.MEW),
    CHIKORITA(Generation2.CHIKORITA),
    BAYLEEF(Generation2.BAYLEEF),
    MEGANIUM(Generation2.MEGANIUM),
    CYNDAQUIL(Generation2.CYNDAQUIL),
    QUILAVA(Generation2.QUILAVA),
    TYPHLOSION(Generation2.TYPHLOSION),
    TOTODILE(Generation2.TOTODILE),
    CROCONAW(Generation2.CROCONAW),
    FERALIGATR(Generation2.FERALIGATR),
    SENTRET(Generation2.SENTRET),
    FURRET(Generation2.FURRET),
    HOOTHOOT(Generation2.HOOTHOOT),
    NOCTOWL(Generation2.NOCTOWL),
    LEDYBA(Generation2.LEDYBA),
    LEDIAN(Generation2.LEDIAN),
    SPINARAK(Generation2.SPINARAK),
    ARIADOS(Generation2.ARIADOS),
    CROBAT(Generation2.CROBAT),
    CHINCHOU(Generation2.CHINCHOU),
    LANTURN(Generation2.LANTURN),
    PICHU(Generation2.PICHU),
    CLEFFA(Generation2.CLEFFA),
    IGGLYBUFF(Generation2.IGGLYBUFF),
    TOGEPI(Generation2.TOGEPI),
    TOGETIC(Generation2.TOGETIC),
    NATU(Generation2.NATU),
    XATU(Generation2.XATU),
    MAREEP(Generation2.MAREEP),
    FLAAFFY(Generation2.FLAAFFY),
    AMPHAROS(Generation2.AMPHAROS),
    BELLOSSOM(Generation2.BELLOSSOM),
    MARILL(Generation2.MARILL),
    AZUMARILL(Generation2.AZUMARILL),
    SUDOWOODO(Generation2.SUDOWOODO),
    POLITOED(Generation2.POLITOED),
    HOPPIP(Generation2.HOPPIP),
    SKIPLOOM(Generation2.SKIPLOOM),
    JUMPLUFF(Generation2.JUMPLUFF),
    AIPOM(Generation2.AIPOM),
    SUNKERN(Generation2.SUNKERN),
    SUNFLORA(Generation2.SUNFLORA),
    YANMA(Generation2.YANMA),
    WOOPER(Generation2.WOOPER),
    QUAGSIRE(Generation2.QUAGSIRE),
    ESPEON(Generation2.ESPEON),
    UMBREON(Generation2.UMBREON),
    MURKROW(Generation2.MURKROW),
    SLOWKING(Generation2.SLOWKING),
    MISDREAVUS(Generation2.MISDREAVUS),
    UNOWN(Generation2.UNOWN),
    WOBBUFFET(Generation2.WOBBUFFET),
    GIRAFARIG(Generation2.GIRAFARIG),
    PINECO(Generation2.PINECO),
    FORRETRESS(Generation2.FORRETRESS),
    DUNSPARCE(Generation2.DUNSPARCE),
    GLIGAR(Generation2.GLIGAR),
    STEELIX(Generation2.STEELIX),
    SNUBBULL(Generation2.SNUBBULL),
    GRANBULL(Generation2.GRANBULL),
    QWILFISH(Generation2.QWILFISH),
    SCIZOR(Generation2.SCIZOR),
    SHUCKLE(Generation2.SHUCKLE),
    HERACROSS(Generation2.HERACROSS),
    SNEASEL(Generation2.SNEASEL),
    TEDDIURSA(Generation2.TEDDIURSA),
    URSARING(Generation2.URSARING),
    SLUGMA(Generation2.SLUGMA),
    MAGCARGO(Generation2.MAGCARGO),
    SWINUB(Generation2.SWINUB),
    PILOSWINE(Generation2.PILOSWINE),
    CORSOLA(Generation2.CORSOLA),
    REMORAID(Generation2.REMORAID),
    OCTILLERY(Generation2.OCTILLERY),
    DELIBIRD(Generation2.DELIBIRD),
    MANTINE(Generation2.MANTINE),
    SKARMORY(Generation2.SKARMORY),
    HOUNDOUR(Generation2.HOUNDOUR),
    HOUNDOOM(Generation2.HOUNDOOM),
    KINGDRA(Generation2.KINGDRA),
    PHANPY(Generation2.PHANPY),
    DONPHAN(Generation2.DONPHAN),
    PORYGON2(Generation2.PORYGON2),
    STANTLER(Generation2.STANTLER),
    SMEARGLE(Generation2.SMEARGLE),
    TYROGUE(Generation2.TYROGUE),
    HITMONTOP(Generation2.HITMONTOP),
    SMOOCHUM(Generation2.SMOOCHUM),
    ELEKID(Generation2.ELEKID),
    MAGBY(Generation2.MAGBY),
    MILTANK(Generation2.MILTANK),
    BLISSEY(Generation2.BLISSEY),
    RAIKOU(Generation2.RAIKOU),
    ENTEI(Generation2.ENTEI),
    SUICUNE(Generation2.SUICUNE),
    LARVITAR(Generation2.LARVITAR),
    PUPITAR(Generation2.PUPITAR),
    TYRANITAR(Generation2.TYRANITAR),
    LUGIA(Generation2.LUGIA),
    HO_OH(Generation2.HO_OH),
    CELEBI(Generation2.CELEBI),
    TREECKO(Generation3.TREECKO),
    GROVYLE(Generation3.GROVYLE),
    SCEPTILE(Generation3.SCEPTILE),
    TORCHIC(Generation3.TORCHIC),
    COMBUSKEN(Generation3.COMBUSKEN),
    BLAZIKEN(Generation3.BLAZIKEN),
    MUDKIP(Generation3.MUDKIP),
    MARSHTOMP(Generation3.MARSHTOMP),
    SWAMPERT(Generation3.SWAMPERT),
    POOCHYENA(Generation3.POOCHYENA),
    MIGHTYENA(Generation3.MIGHTYENA),
    ZIGZAGOON(Generation3.ZIGZAGOON),
    LINOONE(Generation3.LINOONE),
    WURMPLE(Generation3.WURMPLE),
    SILCOON(Generation3.SILCOON),
    BEAUTIFLY(Generation3.BEAUTIFLY),
    CASCOON(Generation3.CASCOON),
    DUSTOX(Generation3.DUSTOX),
    LOTAD(Generation3.LOTAD),
    LOMBRE(Generation3.LOMBRE),
    LUDICOLO(Generation3.LUDICOLO),
    SEEDOT(Generation3.SEEDOT),
    NUZLEAF(Generation3.NUZLEAF),
    SHIFTRY(Generation3.SHIFTRY),
    TAILLOW(Generation3.TAILLOW),
    SWELLOW(Generation3.SWELLOW),
    WINGULL(Generation3.WINGULL),
    PELIPPER(Generation3.PELIPPER),
    RALTS(Generation3.RALTS),
    KIRLIA(Generation3.KIRLIA),
    GARDEVOIR(Generation3.GARDEVOIR),
    SURSKIT(Generation3.SURSKIT),
    MASQUERAIN(Generation3.MASQUERAIN),
    SHROOMISH(Generation3.SHROOMISH),
    BRELOOM(Generation3.BRELOOM),
    SLAKOTH(Generation3.SLAKOTH),
    VIGOROTH(Generation3.VIGOROTH),
    SLAKING(Generation3.SLAKING),
    NINCADA(Generation3.NINCADA),
    NINJASK(Generation3.NINJASK),
    SHEDINJA(Generation3.SHEDINJA),
    WHISMUR(Generation3.WHISMUR),
    LOUDRED(Generation3.LOUDRED),
    EXPLOUD(Generation3.EXPLOUD),
    MAKUHITA(Generation3.MAKUHITA),
    HARIYAMA(Generation3.HARIYAMA),
    AZURILL(Generation3.AZURILL),
    NOSEPASS(Generation3.NOSEPASS),
    SKITTY(Generation3.SKITTY),
    DELCATTY(Generation3.DELCATTY),
    SABLEYE(Generation3.SABLEYE),
    MAWILE(Generation3.MAWILE),
    ARON(Generation3.ARON),
    LAIRON(Generation3.LAIRON),
    AGGRON(Generation3.AGGRON),
    MEDITITE(Generation3.MEDITITE),
    MEDICHAM(Generation3.MEDICHAM),
    ELECTRIKE(Generation3.ELECTRIKE),
    MANECTRIC(Generation3.MANECTRIC),
    PLUSLE(Generation3.PLUSLE),
    MINUN(Generation3.MINUN),
    VOLBEAT(Generation3.VOLBEAT),
    ILLUMISE(Generation3.ILLUMISE),
    ROSELIA(Generation3.ROSELIA),
    GULPIN(Generation3.GULPIN),
    SWALOT(Generation3.SWALOT),
    CARVANHA(Generation3.CARVANHA),
    SHARPEDO(Generation3.SHARPEDO),
    WAILMER(Generation3.WAILMER),
    WAILORD(Generation3.WAILORD),
    NUMEL(Generation3.NUMEL),
    CAMERUPT(Generation3.CAMERUPT),
    TORKOAL(Generation3.TORKOAL),
    SPOINK(Generation3.SPOINK),
    GRUMPIG(Generation3.GRUMPIG),
    SPINDA(Generation3.SPINDA),
    TRAPINCH(Generation3.TRAPINCH),
    VIBRAVA(Generation3.VIBRAVA),
    FLYGON(Generation3.FLYGON),
    CACNEA(Generation3.CACNEA),
    CACTURNE(Generation3.CACTURNE),
    SWABLU(Generation3.SWABLU),
    ALTARIA(Generation3.ALTARIA),
    ZANGOOSE(Generation3.ZANGOOSE),
    SEVIPER(Generation3.SEVIPER),
    LUNATONE(Generation3.LUNATONE),
    SOLROCK(Generation3.SOLROCK),
    BARBOACH(Generation3.BARBOACH),
    WHISCASH(Generation3.WHISCASH),
    CORPHISH(Generation3.CORPHISH),
    CRAWDAUNT(Generation3.CRAWDAUNT),
    BALTOY(Generation3.BALTOY),
    CLAYDOL(Generation3.CLAYDOL),
    LILEEP(Generation3.LILEEP),
    CRADILY(Generation3.CRADILY),
    ANORITH(Generation3.ANORITH),
    ARMALDO(Generation3.ARMALDO),
    FEEBAS(Generation3.FEEBAS),
    MILOTIC(Generation3.MILOTIC),
    CASTFORM(Generation3.CASTFORM),
    CASTFORM_SUNNY(Generation3.CASTFORM_SUNNY),
    CASTFORM_RAINY(Generation3.CASTFORM_RAINY),
    CASTFORM_SNOWY(Generation3.CASTFORM_SNOWY),
    KECLEON(Generation3.KECLEON),
    SHUPPET(Generation3.SHUPPET),
    BANETTE(Generation3.BANETTE),
    DUSKULL(Generation3.DUSKULL),
    DUSCLOPS(Generation3.DUSCLOPS),
    TROPIUS(Generation3.TROPIUS),
    CHIMECHO(Generation3.CHIMECHO),
    ABSOL(Generation3.ABSOL),
    WYNAUT(Generation3.WYNAUT),
    SNORUNT(Generation3.SNORUNT),
    GLALIE(Generation3.GLALIE),
    SPHEAL(Generation3.SPHEAL),
    SEALEO(Generation3.SEALEO),
    WALREIN(Generation3.WALREIN),
    CLAMPERL(Generation3.CLAMPERL),
    HUNTAIL(Generation3.HUNTAIL),
    GOREBYSS(Generation3.GOREBYSS),
    RELICANTH(Generation3.RELICANTH),
    LUVDISC(Generation3.LUVDISC),
    BAGON(Generation3.BAGON),
    SHELGON(Generation3.SHELGON),
    SALAMENCE(Generation3.SALAMENCE),
    BELDUM(Generation3.BELDUM),
    METANG(Generation3.METANG),
    METAGROSS(Generation3.METAGROSS),
    REGIROCK(Generation3.REGIROCK),
    REGICE(Generation3.REGICE),
    REGISTEEL(Generation3.REGISTEEL),
    LATIAS(Generation3.LATIAS),
    LATIOS(Generation3.LATIOS),
    KYOGRE(Generation3.KYOGRE),
    GROUDON(Generation3.GROUDON),
    RAYQUAZA(Generation3.RAYQUAZA),
    JIRACHI(Generation3.JIRACHI),
    DEOXYS_NORMAL(Generation3.DEOXYS_NORMAL),
    DEOXYS_SPEED(Generation3.DEOXYS_SPEED),
    DEOXYS_ATTACK(Generation3.DEOXYS_ATTACK),
    DEOXYS_DEFENSE(Generation3.DEOXYS_DEFENSE),
    TURTWIG(Generation4.TURTWIG),
    GROTLE(Generation4.GROTLE),
    TORTERRA(Generation4.TORTERRA),
    CHIMCHAR(Generation4.CHIMCHAR),
    MONFERNO(Generation4.MONFERNO),
    INFERNAPE(Generation4.INFERNAPE),
    PIPLUP(Generation4.PIPLUP),
    PRINPLUP(Generation4.PRINPLUP),
    EMPOLEON(Generation4.EMPOLEON),
    STARLY(Generation4.STARLY),
    STARAVIA(Generation4.STARAVIA),
    STARAPTOR(Generation4.STARAPTOR),
    BIDOOF(Generation4.BIDOOF),
    BIBAREL(Generation4.BIBAREL),
    KRICKETOT(Generation4.KRICKETOT),
    KRICKETUNE(Generation4.KRICKETUNE),
    SHINX(Generation4.SHINX),
    LUXIO(Generation4.LUXIO),
    LUXRAY(Generation4.LUXRAY),
    BUDEW(Generation4.BUDEW),
    ROSERADE(Generation4.ROSERADE),
    CRANIDOS(Generation4.CRANIDOS),
    RAMPARDOS(Generation4.RAMPARDOS),
    SHIELDON(Generation4.SHIELDON),
    BASTIODON(Generation4.BASTIODON),
    BURMY(Generation4.BURMY),
    WORMADAM(Generation4.WORMADAM),
    MOTHIM(Generation4.MOTHIM),
    COMBEE(Generation4.COMBEE),
    VESPIQUEN(Generation4.VESPIQUEN),
    PACHIRISU(Generation4.PACHIRISU),
    BUIZEL(Generation4.BUIZEL),
    FLOATZEL(Generation4.FLOATZEL),
    CHERUBI(Generation4.CHERUBI),
    CHERRIM(Generation4.CHERRIM),
    SHELLOS(Generation4.SHELLOS),
    GASTRODON(Generation4.GASTRODON),
    AMBIPOM(Generation4.AMBIPOM),
    DRIFLOON(Generation4.DRIFLOON),
    DRIFBLIM(Generation4.DRIFBLIM),
    BUNEARY(Generation4.BUNEARY),
    LOPUNNY(Generation4.LOPUNNY),
    MISMAGIUS(Generation4.MISMAGIUS),
    HONCHKROW(Generation4.HONCHKROW),
    GLAMEOW(Generation4.GLAMEOW),
    PURUGLY(Generation4.PURUGLY),
    CHINGLING(Generation4.CHINGLING),
    STUNKY(Generation4.STUNKY),
    SKUNTANK(Generation4.SKUNTANK),
    BRONZOR(Generation4.BRONZOR),
    BRONZONG(Generation4.BRONZONG),
    BONSLY(Generation4.BONSLY),
    MIME_JR(Generation4.MIME_JR),
    HAPPINY(Generation4.HAPPINY),
    CHATOT(Generation4.CHATOT),
    SPIRITOMB(Generation4.SPIRITOMB),
    GIBLE(Generation4.GIBLE),
    GABITE(Generation4.GABITE),
    GARCHOMP(Generation4.GARCHOMP),
    MUNCHLAX(Generation4.MUNCHLAX),
    RIOLU(Generation4.RIOLU),
    LUCARIO(Generation4.LUCARIO),
    HIPPOPOTAS(Generation4.HIPPOPOTAS),
    HIPPOWDON(Generation4.HIPPOWDON),
    SKORUPI(Generation4.SKORUPI),
    DRAPION(Generation4.DRAPION),
    CROAGUNK(Generation4.CROAGUNK),
    TOXICROAK(Generation4.TOXICROAK),
    CARNIVINE(Generation4.CARNIVINE),
    FINNEON(Generation4.FINNEON),
    LUMINEON(Generation4.LUMINEON),
    MANTYKE(Generation4.MANTYKE),
    SNOVER(Generation4.SNOVER),
    ABOMASNOW(Generation4.ABOMASNOW),
    WEAVILE(Generation4.WEAVILE),
    MAGNEZONE(Generation4.MAGNEZONE),
    LICKILICKY(Generation4.LICKILICKY),
    RHYPERIOR(Generation4.RHYPERIOR),
    TANGROWTH(Generation4.TANGROWTH),
    ELECTIVIRE(Generation4.ELECTIVIRE),
    MAGMORTAR(Generation4.MAGMORTAR),
    TOGEKISS(Generation4.TOGEKISS),
    YANMEGA(Generation4.YANMEGA),
    LEAFEON(Generation4.LEAFEON),
    GLACEON(Generation4.GLACEON),
    GLISCOR(Generation4.GLISCOR),
    MAMOSWINE(Generation4.MAMOSWINE),
    PORYGON_Z(Generation4.PORYGON_Z),
    GALLADE(Generation4.GALLADE),
    PROBOPASS(Generation4.PROBOPASS),
    DUSKNOIR(Generation4.DUSKNOIR),
    FROSLASS(Generation4.FROSLASS),
    ROTOM(Generation4.ROTOM),
    ROTOM_FAN(Generation4.ROTOM_FAN),
    ROTOM_FROST(Generation4.ROTOM_FROST),
    ROTOM_HEAT(Generation4.ROTOM_HEAT),
    ROTOM_MOW(Generation4.ROTOM_MOW),
    ROTOM_WASH(Generation4.ROTOM_WASH),
    UXIE(Generation4.UXIE),
    MESPRIT(Generation4.MESPRIT),
    AZELF(Generation4.AZELF),
    DIALGA(Generation4.DIALGA),
    PALKIA(Generation4.PALKIA),
    HEATRAN(Generation4.HEATRAN),
    REGIGIGAS(Generation4.REGIGIGAS),
    GIRATINA_ALTERED(Generation4.GIRATINA_ALTERED),
    GIRATINA_ORIGIN(Generation4.GIRATINA_ORIGIN),
    CRESSELIA(Generation4.CRESSELIA),
    PHIONE(Generation4.PHIONE),
    MANAPHY(Generation4.MANAPHY),
    DARKRAI(Generation4.DARKRAI),
    SHAYMIN_LAND(Generation4.SHAYMIN_LAND),
    SHAYMIN_SKY(Generation4.SHAYMIN_SKY),
    ARCEUS(Generation4.ARCEUS),
    VICTINI(Generation5.VICTINI),
    SNIVY(Generation5.SNIVY),
    SERVINE(Generation5.SERVINE),
    SERPERIOR(Generation5.SERPERIOR),
    TEPIG(Generation5.TEPIG),
    PIGNITE(Generation5.PIGNITE),
    EMBOAR(Generation5.EMBOAR),
    OSHAWOTT(Generation5.OSHAWOTT),
    DEWOTT(Generation5.DEWOTT),
    SAMUROTT(Generation5.SAMUROTT),
    PATRAT(Generation5.PATRAT),
    WATCHOG(Generation5.WATCHOG),
    LILLIPUP(Generation5.LILLIPUP),
    HERDIER(Generation5.HERDIER),
    STOUTLAND(Generation5.STOUTLAND),
    PURRLOIN(Generation5.PURRLOIN),
    LIEPARD(Generation5.LIEPARD),
    PANSAGE(Generation5.PANSAGE),
    SIMISAGE(Generation5.SIMISAGE),
    PANSEAR(Generation5.PANSEAR),
    SIMISEAR(Generation5.SIMISEAR),
    PANPOUR(Generation5.PANPOUR),
    SIMIPOUR(Generation5.SIMIPOUR),
    MUNNA(Generation5.MUNNA),
    MUSHARNA(Generation5.MUSHARNA),
    PIDOVE(Generation5.PIDOVE),
    TRANQUILL(Generation5.TRANQUILL),
    UNFEZANT(Generation5.UNFEZANT),
    BLITZLE(Generation5.BLITZLE),
    ZEBSTRIKA(Generation5.ZEBSTRIKA),
    ROGGENROLA(Generation5.ROGGENROLA),
    BOLDORE(Generation5.BOLDORE),
    GIGALITH(Generation5.GIGALITH),
    WOOBAT(Generation5.WOOBAT),
    SWOOBAT(Generation5.SWOOBAT),
    DRILBUR(Generation5.DRILBUR),
    EXCADRILL(Generation5.EXCADRILL),
    AUDINO(Generation5.AUDINO),
    TIMBURR(Generation5.TIMBURR),
    GURDURR(Generation5.GURDURR),
    CONKELDURR(Generation5.CONKELDURR),
    TYMPOLE(Generation5.TYMPOLE),
    PALPITOAD(Generation5.PALPITOAD),
    SEISMITOAD(Generation5.SEISMITOAD),
    THROH(Generation5.THROH),
    SAWK(Generation5.SAWK),
    SEWADDLE(Generation5.SEWADDLE),
    SWADLOON(Generation5.SWADLOON),
    LEAVANNY(Generation5.LEAVANNY),
    VENIPEDE(Generation5.VENIPEDE),
    WHIRLIPEDE(Generation5.WHIRLIPEDE),
    SCOLIPEDE(Generation5.SCOLIPEDE),
    COTTONEE(Generation5.COTTONEE),
    WHIMSICOTT(Generation5.WHIMSICOTT),
    PETILIL(Generation5.PETILIL),
    LILLIGANT(Generation5.LILLIGANT),
    BASCULIN(Generation5.BASCULIN),
    SANDILE(Generation5.SANDILE),
    KROKOROK(Generation5.KROKOROK),
    KROOKODILE(Generation5.KROOKODILE),
    DARUMAKA(Generation5.DARUMAKA),
    DARMANITAN_STANDARD(Generation5.DARMANITAN_STANDARD),
    DARMANITAN_ZEN(Generation5.DARMANITAN_ZEN),
    MARACTUS(Generation5.MARACTUS),
    DWEBBLE(Generation5.DWEBBLE),
    CRUSTLE(Generation5.CRUSTLE),
    SCRAGGY(Generation5.SCRAGGY),
    SCRAFTY(Generation5.SCRAFTY),
    SIGILYPH(Generation5.SIGILYPH),
    YAMASK(Generation5.YAMASK),
    COFAGRIGUS(Generation5.COFAGRIGUS),
    TIRTOUGA(Generation5.TIRTOUGA),
    CARRACOSTA(Generation5.CARRACOSTA),
    ARCHEN(Generation5.ARCHEN),
    ARCHEOPS(Generation5.ARCHEOPS),
    TRUBBISH(Generation5.TRUBBISH),
    GARBODOR(Generation5.GARBODOR),
    ZORUA(Generation5.ZORUA),
    ZOROARK(Generation5.ZOROARK),
    MINCCINO(Generation5.MINCCINO),
    CINCCINO(Generation5.CINCCINO),
    GOTHITA(Generation5.GOTHITA),
    GOTHORITA(Generation5.GOTHORITA),
    GOTHITELLE(Generation5.GOTHITELLE),
    SOLOSIS(Generation5.SOLOSIS),
    DUOSION(Generation5.DUOSION),
    REUNICLUS(Generation5.REUNICLUS),
    DUCKLETT(Generation5.DUCKLETT),
    SWANNA(Generation5.SWANNA),
    VANILLITE(Generation5.VANILLITE),
    VANILLISH(Generation5.VANILLISH),
    VANILLUXE(Generation5.VANILLUXE),
    DEERLING(Generation5.DEERLING),
    SAWSBUCK(Generation5.SAWSBUCK),
    EMOLGA(Generation5.EMOLGA),
    KARRABLAST(Generation5.KARRABLAST),
    ESCAVALIER(Generation5.ESCAVALIER),
    FOONGUS(Generation5.FOONGUS),
    AMOONGUSS(Generation5.AMOONGUSS),
    FRILLISH(Generation5.FRILLISH),
    JELLICENT(Generation5.JELLICENT),
    ALOMOMOLA(Generation5.ALOMOMOLA),
    JOLTIK(Generation5.JOLTIK),
    GALVANTULA(Generation5.GALVANTULA),
    FERROSEED(Generation5.FERROSEED),
    FERROTHORN(Generation5.FERROTHORN),
    KLINK(Generation5.KLINK),
    KLANG(Generation5.KLANG),
    KLINKLANG(Generation5.KLINKLANG),
    TYNAMO(Generation5.TYNAMO),
    EELEKTRIK(Generation5.EELEKTRIK),
    EELEKTROSS(Generation5.EELEKTROSS),
    ELGYEM(Generation5.ELGYEM),
    BEHEEYEM(Generation5.BEHEEYEM),
    LITWICK(Generation5.LITWICK),
    LAMPENT(Generation5.LAMPENT),
    CHANDELURE(Generation5.CHANDELURE),
    AXEW(Generation5.AXEW),
    FRAXURE(Generation5.FRAXURE),
    HAXORUS(Generation5.HAXORUS),
    CUBCHOO(Generation5.CUBCHOO),
    BEARTIC(Generation5.BEARTIC),
    CRYOGONAL(Generation5.CRYOGONAL),
    SHELMET(Generation5.SHELMET),
    ACCELGOR(Generation5.ACCELGOR),
    STUNFISK(Generation5.STUNFISK),
    MIENFOO(Generation5.MIENFOO),
    MIENSHAO(Generation5.MIENSHAO),
    DRUDDIGON(Generation5.DRUDDIGON),
    GOLETT(Generation5.GOLETT),
    GOLURK(Generation5.GOLURK),
    PAWNIARD(Generation5.PAWNIARD),
    BISHARP(Generation5.BISHARP),
    BOUFFALANT(Generation5.BOUFFALANT),
    RUFFLET(Generation5.RUFFLET),
    BRAVIARY(Generation5.BRAVIARY),
    VULLABY(Generation5.VULLABY),
    MANDIBUZZ(Generation5.MANDIBUZZ),
    HEATMOR(Generation5.HEATMOR),
    DURANT(Generation5.DURANT),
    DEINO(Generation5.DEINO),
    ZWEILOUS(Generation5.ZWEILOUS),
    HYDREIGON(Generation5.HYDREIGON),
    LARVESTA(Generation5.LARVESTA),
    VOLCARONA(Generation5.VOLCARONA),
    COBALION(Generation5.COBALION),
    TERRAKION(Generation5.TERRAKION),
    VIRIZION(Generation5.VIRIZION),
    TORNADUS_INCARNATE(Generation5.TORNADUS_INCARNATE),
    TORNADUS_THERIAN(Generation5.TORNADUS_THERIAN),
    THUNDURUS_INCARNATE(Generation5.THUNDURUS_INCARNATE),
    THUNDURUS_THERIAN(Generation5.THUNDURUS_THERIAN),
    RESHIRAM(Generation5.RESHIRAM),
    ZEKROM(Generation5.ZEKROM),
    LANDORUS_INCARNATE(Generation5.LANDORUS_INCARNATE),
    LANDORUS_THERIAN(Generation5.LANDORUS_THERIAN),
    KYUREM(Generation5.KYUREM),
    KYUREM_BLACK(Generation5.KYUREM_BLACK),
    KYUREM_WHITE(Generation5.KYUREM_WHITE),
    KELDEO(Generation5.KELDEO),
    KELDEO_RESOLUTE(Generation5.KELDEO_RESOLUTE),
    MELOETTA_ARIA(Generation5.MELOETTA_ARIA),
    MELOETTA_PIROUETTE(Generation5.MELOETTA_PIROUETTE),
    GENESECT(Generation5.GENESECT),
    CHESPIN(Generation6.CHESPIN),
    QUILLADIN(Generation6.QUILLADIN),
    CHESNAUGHT(Generation6.CHESNAUGHT),
    FENNEKIN(Generation6.FENNEKIN),
    BRAIXEN(Generation6.BRAIXEN),
    DELPHOX(Generation6.DELPHOX),
    FROAKIE(Generation6.FROAKIE),
    FROGADIER(Generation6.FROGADIER),
    GRENINJA(Generation6.GRENINJA),
    BUNNELBY(Generation6.BUNNELBY),
    DIGGERSBY(Generation6.DIGGERSBY),
    FLETCHLING(Generation6.FLETCHLING),
    FLETCHINDER(Generation6.FLETCHINDER),
    TALONFLAME(Generation6.TALONFLAME),
    SCATTERBUG(Generation6.SCATTERBUG),
    SPEWPA(Generation6.SPEWPA),
    VIVILLON(Generation6.VIVILLON),
    LITLEO(Generation6.LITLEO),
    PYROAR(Generation6.PYROAR),
    FLABEBE(Generation6.FLABEBE),
    FLOETTE(Generation6.FLOETTE),
    FLORGES(Generation6.FLORGES),
    SKIDDO(Generation6.SKIDDO),
    GOGOAT(Generation6.GOGOAT),
    PANCHAM(Generation6.PANCHAM),
    PANGORO(Generation6.PANGORO),
    FURFROU(Generation6.FURFROU),
    ESPURR(Generation6.ESPURR),
    MEOWSTIC_MALE(Generation6.MEOWSTIC_MALE),
    MEOWSTIC_FEMALE(Generation6.MEOWSTIC_FEMALE),
    HONEDGE(Generation6.HONEDGE),
    DOUBLADE(Generation6.DOUBLADE),
    AEGISLASH(Generation6.AEGISLASH),
    SPRITZEE(Generation6.SPRITZEE),
    AROMATISSE(Generation6.AROMATISSE),
    SWIRLIX(Generation6.SWIRLIX),
    SLURPUFF(Generation6.SLURPUFF),
    INKAY(Generation6.INKAY),
    MALAMAR(Generation6.MALAMAR),
    BINACLE(Generation6.BINACLE),
    BARBARACLE(Generation6.BARBARACLE),
    SKRELP(Generation6.SKRELP),
    DRAGALGE(Generation6.DRAGALGE),
    CLAUNCHER(Generation6.CLAUNCHER),
    CLAWITZER(Generation6.CLAWITZER),
    HELIOPTILE(Generation6.HELIOPTILE),
    HELIOLISK(Generation6.HELIOLISK),
    TYRUNT(Generation6.TYRUNT),
    TYRANTRUM(Generation6.TYRANTRUM),
    AMAURA(Generation6.AMAURA),
    AURORUS(Generation6.AURORUS),
    SYLVEON(Generation6.SYLVEON),
    HAWLUCHA(Generation6.HAWLUCHA),
    DEDENNE(Generation6.DEDENNE),
    CARBINK(Generation6.CARBINK),
    GOOMY(Generation6.GOOMY),
    SLIGGOO(Generation6.SLIGGOO),
    GOODRA(Generation6.GOODRA),
    KLEFKI(Generation6.KLEFKI),
    PHANTUMP(Generation6.PHANTUMP),
    TREVENANT(Generation6.TREVENANT),
    PUMPKABOO(Generation6.PUMPKABOO),
    GOURGEIST(Generation6.GOURGEIST),
    BERGMITE(Generation6.BERGMITE),
    AVALUGG(Generation6.AVALUGG),
    NOIBAT(Generation6.NOIBAT),
    NOIVERN(Generation6.NOIVERN),
    XERNEAS(Generation6.XERNEAS),
    YVELTAL(Generation6.YVELTAL),
    ZYGARDE_50(Generation6.ZYGARDE_50),
    DIANCIE(Generation6.DIANCIE),
    HOOPA(Generation6.HOOPA),
    VOLCANION(Generation6.VOLCANION),

    MEGA_VENUSAUR(Generation6.MEGA_VENUSAUR),
    MEGA_CHARIZARD_X(Generation6.MEGA_CHARIZARD_X),
    MEGA_CHARIZARD_Y(Generation6.MEGA_CHARIZARD_Y),
    MEGA_BLASTOISE(Generation6.MEGA_BLASTOISE),
    MEGA_ALAKAZAM(Generation6.MEGA_ALAKAZAM),
    MEGA_GENGAR(Generation6.MEGA_GENGAR),
    MEGA_KANGASKHAN(Generation6.MEGA_KANGASKHAN),
    MEGA_PINSIR(Generation6.MEGA_PINSIR),
    MEGA_GYARADOS(Generation6.MEGA_GYARADOS),
    MEGA_AERODACTYL(Generation6.MEGA_AERODACTYL),
    MEGA_MEWTWO_X(Generation6.MEGA_MEWTWO_X),
    MEGA_MEWTWO_Y(Generation6.MEGA_MEWTWO_Y),
    MEGA_AMPHAROS(Generation6.MEGA_AMPHAROS),
    MEGA_SCIZOR(Generation6.MEGA_SCIZOR),
    MEGA_HERACROSS(Generation6.MEGA_HERACROSS),
    MEGA_HOUNDOOM(Generation6.MEGA_HOUNDOOM),
    MEGA_TYRANITAR(Generation6.MEGA_TYRANITAR),
    MEGA_BLAZIKEN(Generation6.MEGA_BLAZIKEN),
    MEGA_GARDEVOIR(Generation6.MEGA_GARDEVOIR),
    MEGA_MAWILE(Generation6.MEGA_MAWILE),
    MEGA_AGGRON(Generation6.MEGA_AGGRON),
    MEGA_MEDICHAM(Generation6.MEGA_MEDICHAM),
    MEGA_MANECTRIC(Generation6.MEGA_MANECTRIC),
    MEGA_BANETTE(Generation6.MEGA_BANETTE),
    MEGA_ABSOL(Generation6.MEGA_ABSOL),
    MEGA_GARCHOMP(Generation6.MEGA_GARCHOMP),
    MEGA_LUCARIO(Generation6.MEGA_LUCARIO),
    MEGA_ABOMASNOW(Generation6.MEGA_ABOMASNOW),
    MEGA_LATIAS(Generation6.MEGA_LATIAS),
    MEGA_LATIOS(Generation6.MEGA_LATIOS),
    MEGA_SWAMPERT(Generation6.MEGA_SWAMPERT),
    MEGA_SCEPTILE(Generation6.MEGA_SCEPTILE),
    MEGA_SABLEYE(Generation6.MEGA_SABLEYE),
    MEGA_ALTARIA(Generation6.MEGA_ALTARIA),
    MEGA_GALLADE(Generation6.MEGA_GALLADE),
    MEGA_AUDINO(Generation6.MEGA_AUDINO),
    MEGA_SHARPEDO(Generation6.MEGA_SHARPEDO),
    MEGA_SLOWBRO(Generation6.MEGA_SLOWBRO),
    MEGA_STEELIX(Generation6.MEGA_STEELIX),
    MEGA_PIDGEOT(Generation6.MEGA_PIDGEOT),
    MEGA_GLALIE(Generation6.MEGA_GLALIE),
    MEGA_DIANCIE(Generation6.MEGA_DIANCIE),
    MEGA_METAGROSS(Generation6.MEGA_METAGROSS),
    PRIMAL_KYOGRE(Generation6.PRIMAL_KYOGRE),
    PRIMAL_GROUDON(Generation6.PRIMAL_GROUDON),
    MEGA_RAYQUAZA(Generation6.MEGA_RAYQUAZA),
    HOOPA_UNBOUND(Generation6.HOOPA_UNBOUND),
    MEGA_CAMERUPT(Generation6.MEGA_CAMERUPT),
    MEGA_LOPUNNY(Generation6.MEGA_LOPUNNY),
    MEGA_SALAMENCE(Generation6.MEGA_SALAMENCE),
    MEGA_BEEDRILL(Generation6.MEGA_BEEDRILL),

    ROWLET(Generation7.ROWLET),
    DARTRIX(Generation7.DARTRIX),
    DECIDUEYE(Generation7.DECIDUEYE),
    LITTEN(Generation7.LITTEN),
    TORRACAT(Generation7.TORRACAT),
    INCINEROAR(Generation7.INCINEROAR),
    POPPLIO(Generation7.POPPLIO),
    BRIONNE(Generation7.BRIONNE),
    PRIMARINA(Generation7.PRIMARINA),
    PIKIPEK(Generation7.PIKIPEK),
    TRUMBEAK(Generation7.TRUMBEAK),
    TOUCANNON(Generation7.TOUCANNON),
    YUNGOOS(Generation7.YUNGOOS),
    GUMSHOOS(Generation7.GUMSHOOS),
    GRUBBIN(Generation7.GRUBBIN),
    CHARJABUG(Generation7.CHARJABUG),
    VIKAVOLT(Generation7.VIKAVOLT),
    CRABRAWLER(Generation7.CRABRAWLER),
    CRABOMINABLE(Generation7.CRABOMINABLE),
    ORICORIO_BAILE(Generation7.ORICORIO_BAILE),
    CUTIEFLY(Generation7.CUTIEFLY),
    RIBOMBEE(Generation7.RIBOMBEE),
    ROCKRUFF(Generation7.ROCKRUFF),
    LYCANROC_MIDDAY(Generation7.LYCANROC_MIDDAY),
    WISHIWASHI_SOLO(Generation7.WISHIWASHI_SOLO),
    MAREANIE(Generation7.MAREANIE),
    TOXAPEX(Generation7.TOXAPEX),
    MUDBRAY(Generation7.MUDBRAY),
    MUDSDALE(Generation7.MUDSDALE),
    DEWPIDER(Generation7.DEWPIDER),
    ARAQUANID(Generation7.ARAQUANID),
    FOMANTIS(Generation7.FOMANTIS),
    LURANTIS(Generation7.LURANTIS),
    MORELULL(Generation7.MORELULL),
    SHIINOTIC(Generation7.SHIINOTIC),
    SALANDIT(Generation7.SALANDIT),
    SALAZZLE(Generation7.SALAZZLE),
    STUFFUL(Generation7.STUFFUL),
    BEWEAR(Generation7.BEWEAR),
    BOUNSWEET(Generation7.BOUNSWEET),
    STEENEE(Generation7.STEENEE),
    TSAREENA(Generation7.TSAREENA),
    COMFEY(Generation7.COMFEY),
    ORANGURU(Generation7.ORANGURU),
    PASSIMIAN(Generation7.PASSIMIAN),
    WIMPOD(Generation7.WIMPOD),
    GOLISOPOD(Generation7.GOLISOPOD),
    SANDYGAST(Generation7.SANDYGAST),
    PALOSSAND(Generation7.PALOSSAND),
    PYUKUMUKU(Generation7.PYUKUMUKU),
    TYPE_NULL(Generation7.TYPE_NULL),
    SILVALLY(Generation7.SILVALLY),
    MINIOR(Generation7.MINIOR),
    KOMALA(Generation7.KOMALA),
    TURTONATOR(Generation7.TURTONATOR),
    TOGEDEMARU(Generation7.TOGEDEMARU),
    MIMIKYU(Generation7.MIMIKYU),
    BRUXISH(Generation7.BRUXISH),
    DRAMPA(Generation7.DRAMPA),
    DHELMISE(Generation7.DHELMISE),
    JANGMO_O(Generation7.JANGMO_O),
    HAKAMO_O(Generation7.HAKAMO_O),
    KOMMO_O(Generation7.KOMMO_O),
    TAPU_KOKO(Generation7.TAPU_KOKO),
    TAPU_LELE(Generation7.TAPU_LELE),
    TAPU_BULU(Generation7.TAPU_BULU),
    TAPU_FINI(Generation7.TAPU_FINI),
    COSMOG(Generation7.COSMOG),
    COSMOEM(Generation7.COSMOEM),
    SOLGALEO(Generation7.SOLGALEO),
    LUNALA(Generation7.LUNALA),
    NIHILEGO(Generation7.NIHILEGO),
    BUZZWOLE(Generation7.BUZZWOLE),
    PHEROMOSA(Generation7.PHEROMOSA),
    XURKITREE(Generation7.XURKITREE),
    CELESTEELA(Generation7.CELESTEELA),
    KARTANA(Generation7.KARTANA),
    GUZZLORD(Generation7.GUZZLORD),
    NECROZMA(Generation7.NECROZMA),
    MAGEARNA(Generation7.MAGEARNA),
    MARSHADOW(Generation7.MARSHADOW),
    POIPOLE(Generation7.POIPOLE),
    NAGANADEL(Generation7.NAGANADEL),
    STAKATAKA(Generation7.STAKATAKA),
    BLACEPHALON(Generation7.BLACEPHALON),
    ZERAORA(Generation7.ZERAORA),
    MELTAN(Generation7.MELTAN),
    MELMETAL(Generation7.MELMETAL),

    ALOLAN_RATTATA(Generation7.ALOLAN_RATTATA),
    ALOLAN_RATICATE(Generation7.ALOLAN_RATICATE),
    ALOLAN_RAICHU(Generation7.ALOLAN_RAICHU),
    ALOLAN_SANDSHREW(Generation7.ALOLAN_SANDSHREW),
    ALOLAN_SANDSLASH(Generation7.ALOLAN_SANDSLASH),
    ALOLAN_VULPIX(Generation7.ALOLAN_VULPIX),
    ALOLAN_NINETALES(Generation7.ALOLAN_NINETALES),
    ALOLAN_DIGLETT(Generation7.ALOLAN_DIGLETT),
    ALOLAN_DUGTRIO(Generation7.ALOLAN_DUGTRIO),
    ALOLAN_MEOWTH(Generation7.ALOLAN_MEOWTH),
    ALOLAN_PERSIAN(Generation7.ALOLAN_PERSIAN),
    ALOLAN_GEODUDE(Generation7.ALOLAN_GEODUDE),
    ALOLAN_GRAVELER(Generation7.ALOLAN_GRAVELER),
    ALOLAN_GOLEM(Generation7.ALOLAN_GOLEM),
    ALOLAN_GRIMER(Generation7.ALOLAN_GRIMER),
    ALOLAN_MUK(Generation7.ALOLAN_MUK),
    ALOLAN_EXEGGUTOR(Generation7.ALOLAN_EXEGGUTOR),
    ALOLAN_MAROWAK(Generation7.ALOLAN_MAROWAK),
    ASH_GRENINJA(Generation7.ASH_GRENINJA),
    ZYGARDE_10(Generation7.ZYGARDE_10),
    ZYGARDE_100(Generation7.ZYGARDE_100),
    ORICORIO_POMPOM(Generation7.ORICORIO_POMPOM),
    ORICORIO_PAU(Generation7.ORICORIO_PAU),
    ORICORIO_SENSU(Generation7.ORICORIO_SENSU),
    LYCANROC_MIDNIGHT(Generation7.LYCANROC_MIDNIGHT),
    WISHIWASHI_SCHOOL(Generation7.WISHIWASHI_SCHOOL),
    MINIOR_CORE(Generation7.MINIOR_CORE),
    LYCANROC_CREPUSCULAR(Generation7.LYCANROC_CREPUSCULAR),
    DUSK_MANE_NECROZMA(Generation7.DUSK_MANE_NECROZMA),
    DAWN_WINGS_NECROZMA(Generation7.DAWN_WINGS_NECROZMA),
    ULTRA_NECROZMA(Generation7.ULTRA_NECROZMA),

    GROOKEY(Generation8.GROOKEY),
    THWACKEY(Generation8.THWACKEY),
    RILLABOOM(Generation8.RILLABOOM),
    SCORBUNNY(Generation8.SCORBUNNY),
    RABOOT(Generation8.RABOOT),
    CINDERACE(Generation8.CINDERACE),
    SOBBLE(Generation8.SOBBLE),
    DRIZZILE(Generation8.DRIZZILE),
    INTELEON(Generation8.INTELEON),
    SKWOVET(Generation8.SKWOVET),
    GREEDENT(Generation8.GREEDENT),
    ROOKIDEE(Generation8.ROOKIDEE),
    CORVISQUIRE(Generation8.CORVISQUIRE),
    CORVIKNIGHT(Generation8.CORVIKNIGHT),
    BLIPBUG(Generation8.BLIPBUG),
    DOTTLER(Generation8.DOTTLER),
    ORBEETLE(Generation8.ORBEETLE),
    NICKIT(Generation8.NICKIT),
    THIEVUL(Generation8.THIEVUL),
    GOSSIFLEUR(Generation8.GOSSIFLEUR),
    ELDEGOSS(Generation8.ELDEGOSS),
    WOOLOO(Generation8.WOOLOO),
    DUBWOOL(Generation8.DUBWOOL),
    CHEWTLE(Generation8.CHEWTLE),
    DREDNAW(Generation8.DREDNAW),
    YAMPER(Generation8.YAMPER),
    BOLTUND(Generation8.BOLTUND),
    ROLYCOLY(Generation8.ROLYCOLY),
    CARKOL(Generation8.CARKOL),
    COALOSSAL(Generation8.COALOSSAL),
    APPLIN(Generation8.APPLIN),
    FLAPPLE(Generation8.FLAPPLE),
    APPLETUN(Generation8.APPLETUN),
    SILICOBRA(Generation8.SILICOBRA),
    SANDACONDA(Generation8.SANDACONDA),
    CRAMORANT(Generation8.CRAMORANT),
    ARROKUDA(Generation8.ARROKUDA),
    BARRASKEWDA(Generation8.BARRASKEWDA),
    TOXEL(Generation8.TOXEL),
    TOXTRICITY_AMPED(Generation8.TOXTRICITY_AMPED),
    SIZZLIPEDE(Generation8.SIZZLIPEDE),
    CENTISKORCH(Generation8.CENTISKORCH),
    CLOBBOPUS(Generation8.CLOBBOPUS),
    GRAPPLOCT(Generation8.GRAPPLOCT),
    SINISTEA(Generation8.SINISTEA),
    POLTEAGEIST(Generation8.POLTEAGEIST),
    HATENNA(Generation8.HATENNA),
    HATTREM(Generation8.HATTREM),
    HATTERENE(Generation8.HATTERENE),
    IMPIDIMP(Generation8.IMPIDIMP),
    MORGREM(Generation8.MORGREM),
    GRIMMSNARL(Generation8.GRIMMSNARL),
    OBSTAGOON(Generation8.OBSTAGOON),
    PERRSERKER(Generation8.PERRSERKER),
    CURSOLA(Generation8.CURSOLA),
    SIRFETCHD(Generation8.SIRFETCHD),
    MR_RIME(Generation8.MR_RIME),
    RUNERIGUS(Generation8.RUNERIGUS),
    MILCERY(Generation8.MILCERY),
    ALCREMIE(Generation8.ALCREMIE),
    FALINKS(Generation8.FALINKS),
    PINCURCHIN(Generation8.PINCURCHIN),
    SNOM(Generation8.SNOM),
    FROSMOTH(Generation8.FROSMOTH),
    STONJOURNER(Generation8.STONJOURNER),
    EISCUE_ICE(Generation8.EISCUE_ICE),
    INDEEDEE_FEMALE(Generation8.INDEEDEE_FEMALE),
    MORPEKO(Generation8.MORPEKO),
    CUFANT(Generation8.CUFANT),
    COPPERAJAH(Generation8.COPPERAJAH),
    DRACOZOLT(Generation8.DRACOZOLT),
    ARCTOZOLT(Generation8.ARCTOZOLT),
    DRACOVISH(Generation8.DRACOVISH),
    ARCTOVISH(Generation8.ARCTOVISH),
    DURALUDON(Generation8.DURALUDON),
    DREEPY(Generation8.DREEPY),
    DRAKLOAK(Generation8.DRAKLOAK),
    DRAGAPULT(Generation8.DRAGAPULT),
    ZACIAN(Generation8.ZACIAN),
    ZAMAZENTA(Generation8.ZAMAZENTA),
    ETERNATUS(Generation8.ETERNATUS),
    KUBFU(Generation8.KUBFU),
    URSHIFU_SINGLE(Generation8.URSHIFU_SINGLE),
    ZARUDE(Generation8.ZARUDE),
    REGIELEKI(Generation8.REGIELEKI),
    REGIDRAGO(Generation8.REGIDRAGO),
    GLASTRIER(Generation8.GLASTRIER),
    SPECTRIER(Generation8.SPECTRIER),
    CALYREX(Generation8.CALYREX),
    WYRDEER(Generation8.WYRDEER),
    KLEAVOR(Generation8.KLEAVOR),
    URSALUNA(Generation8.URSALUNA),
    BASCULEGION_MALE(Generation8.BASCULEGION_MALE),
    SNEASLER(Generation8.SNEASLER),
    OVERQWIL(Generation8.OVERQWIL),
    ENAMORUS_INCARNATE(Generation8.ENAMORUS_INCARNATE),
    GALARIAN_MEOWTH(Generation8.GALARIAN_MEOWTH),
    GALARIAN_PONYTA(Generation8.GALARIAN_PONYTA),
    GALARIAN_RAPIDASH(Generation8.GALARIAN_RAPIDASH),
    GALARIAN_SLOWPOKE(Generation8.GALARIAN_SLOWPOKE),
    GALARIAN_SLOWBRO(Generation8.GALARIAN_SLOWBRO),
    GALARIAN_FARFETCHD(Generation8.GALARIAN_FARFETCHD),
    GALARIAN_WEEZING(Generation8.GALARIAN_WEEZING),
    GALARIAN_MR_MIME(Generation8.GALARIAN_MR_MIME),
    GALARIAN_ARTICUNO(Generation8.GALARIAN_ARTICUNO),
    GALARIAN_ZAPDOS(Generation8.GALARIAN_ZAPDOS),
    GALARIAN_MOLTRES(Generation8.GALARIAN_MOLTRES),
    GALARIAN_SLOWKING(Generation8.GALARIAN_SLOWKING),
    GALARIAN_CORSOLA(Generation8.GALARIAN_CORSOLA),
    GALARIAN_ZIGZAGOON(Generation8.GALARIAN_ZIGZAGOON),
    GALARIAN_LINOONE(Generation8.GALARIAN_LINOONE),
    GALARIAN_DARUMAKA(Generation8.GALARIAN_DARUMAKA),
    GALARIAN_DARMANITAN(Generation8.GALARIAN_DARMANITAN),
    GALARIAN_DARMANITAN_ZEN(Generation8.GALARIAN_DARMANITAN_ZEN),
    GALARIAN_YAMASK(Generation8.GALARIAN_YAMASK),
    GALARIAN_STUNFISK(Generation8.GALARIAN_STUNFISK),
    TOXTRICITY_LOWKEY(Generation8.TOXTRICITY_LOWKEY),
    INDEEDEE_MALE(Generation8.INDEEDEE_MALE),
    CROWNED_SWORD_ZACIAN(Generation8.CROWNED_SWORD_ZACIAN),
    CROWNED_SHIELD_ZAMAZENTA(Generation8.CROWNED_SHIELD_ZAMAZENTA),
    ETERNAMAX_ETERNATUS(Generation8.ETERNAMAX_ETERNATUS),
    RAPID_STRIKE_URSHIFU(Generation8.RAPID_STRIKE_URSHIFU),
    SHADOW_RIDER_CALYREX(Generation8.SHADOW_RIDER_CALYREX),
    ICE_RIDER_CALYREX(Generation8.ICE_RIDER_CALYREX),
    GIGANTAMAX_VENUSAUR(Generation8.GIGANTAMAX_VENUSAUR),
    GIGANTAMAX_CHARIZARD(Generation8.GIGANTAMAX_CHARIZARD),
    GIGANTAMAX_BLASTOISE(Generation8.GIGANTAMAX_BLASTOISE),
    GIGANTAMAX_BUTTERFREE(Generation8.GIGANTAMAX_BUTTERFREE),
    GIGANTAMAX_PIKACHU(Generation8.GIGANTAMAX_PIKACHU),
    GIGANTAMAX_MEOWTH(Generation8.GIGANTAMAX_MEOWTH),
    GIGANTAMAX_MACHAMP(Generation8.GIGANTAMAX_MACHAMP),
    GIGANTAMAX_GENGAR(Generation8.GIGANTAMAX_GENGAR),
    GIGANTAMAX_KINGLER(Generation8.GIGANTAMAX_KINGLER),
    GIGANTAMAX_LAPRAS(Generation8.GIGANTAMAX_LAPRAS),
    GIGANTAMAX_EEVEE(Generation8.GIGANTAMAX_EEVEE),
    GIGANTAMAX_SNORLAX(Generation8.GIGANTAMAX_SNORLAX),
    GIGANTAMAX_GARBODOR(Generation8.GIGANTAMAX_GARBODOR),
    GIGANTAMAX_MELMETAL(Generation8.GIGANTAMAX_MELMETAL),
    GIGANTAMAX_RILLABOOM(Generation8.GIGANTAMAX_RILLABOOM),
    GIGANTAMAX_CINDERACE(Generation8.GIGANTAMAX_CINDERACE),
    GIGANTAMAX_INTELEON(Generation8.GIGANTAMAX_INTELEON),
    GIGANTAMAX_CORVIKNIGHT(Generation8.GIGANTAMAX_CORVIKNIGHT),
    GIGANTAMAX_ORBEETLE(Generation8.GIGANTAMAX_ORBEETLE),
    GIGANTAMAX_DREDNAW(Generation8.GIGANTAMAX_DREDNAW),
    GIGANTAMAX_COALOSSAL(Generation8.GIGANTAMAX_COALOSSAL),
    GIGANTAMAX_FLAPPLE(Generation8.GIGANTAMAX_FLAPPLE),
    GIGANTAMAX_APPLETUN(Generation8.GIGANTAMAX_APPLETUN),
    GIGANTAMAX_SANDACONDA(Generation8.GIGANTAMAX_SANDACONDA),
    GIGANTAMAX_TOXTRICITY_AMPED(Generation8.GIGANTAMAX_TOXTRICITY_AMPED),
    GIGANTAMAX_TOXTRICITY_LOWKEY(Generation8.GIGANTAMAX_TOXTRICITY_LOWKEY),
    GIGANTAMAX_CENTISKORCH(Generation8.GIGANTAMAX_CENTISKORCH),
    GIGANTAMAX_HATTERENE(Generation8.GIGANTAMAX_HATTERENE),
    GIGANTAMAX_GRIMMSNARL(Generation8.GIGANTAMAX_GRIMMSNARL),
    GIGANTAMAX_ALCREMIE(Generation8.GIGANTAMAX_ALCREMIE),
    GIGANTAMAX_COPPERAJAH(Generation8.GIGANTAMAX_COPPERAJAH),
    GIGANTAMAX_DURALUDON(Generation8.GIGANTAMAX_DURALUDON),
    GIGANTAMAX_URSHIFU_SINGLE(Generation8.GIGANTAMAX_URSHIFU_SINGLE),
    GIGANTAMAX_URSHIFU_RAPID(Generation8.GIGANTAMAX_URSHIFU_RAPID),
    HISUIAN_GROWLITHE(Generation8.HISUIAN_GROWLITHE),
    HISUIAN_ARCANINE(Generation8.HISUIAN_ARCANINE),
    HISUIAN_VOLTORB(Generation8.HISUIAN_VOLTORB),
    HISUIAN_ELECTRODE(Generation8.HISUIAN_ELECTRODE),
    HISUIAN_TYPHLOSION(Generation8.HISUIAN_TYPHLOSION),
    HISUIAN_QWILFISH(Generation8.HISUIAN_QWILFISH),
    HISUIAN_SNEASEL(Generation8.HISUIAN_SNEASEL),
    HISUIAN_SAMUROTT(Generation8.HISUIAN_SAMUROTT),
    HISUIAN_LILLIGANT(Generation8.HISUIAN_LILLIGANT),
    HISUIAN_ZORUA(Generation8.HISUIAN_ZORUA),
    HISUIAN_ZOROARK(Generation8.HISUIAN_ZOROARK),
    HISUIAN_BRAVIARY(Generation8.HISUIAN_BRAVIARY),
    HISUIAN_SLIGGOO(Generation8.HISUIAN_SLIGGOO),
    HISUIAN_GOODRA(Generation8.HISUIAN_GOODRA),
    HISUIAN_AVALUGG(Generation8.HISUIAN_AVALUGG),
    HISUIAN_DECIDUEYE(Generation8.HISUIAN_DECIDUEYE),
    ORIGIN_DIALGA(Generation8.ORIGIN_DIALGA),
    ORIGIN_PALKIA(Generation8.ORIGIN_PALKIA),
    HISUIAN_BASCULIN(Generation8.HISUIAN_BASCULIN),
    BASCULEGION_FEMALE(Generation8.BASCULEGION_FEMALE),
    ENAMORUS_THERIAN(Generation8.ENAMORUS_THERIAN),

    SPRIGATITO(Generation9.SPRIGATITO),
    FLORAGATO(Generation9.FLORAGATO),
    MEOWSCARADA(Generation9.MEOWSCARADA),
    FUECOCO(Generation9.FUECOCO),
    CROCALOR(Generation9.CROCALOR),
    SKELEDIRGE(Generation9.SKELEDIRGE),
    QUAXLY(Generation9.QUAXLY),
    QUAXWELL(Generation9.QUAXWELL),
    QUAQUAVAL(Generation9.QUAQUAVAL),
    LECHONK(Generation9.LECHONK),
    OINKOLOGNE_MALE(Generation9.OINKOLOGNE_MALE),
    OINKOLOGNE_FEMALE(Generation9.OINKOLOGNE_FEMALE),
    TAROUNTULA(Generation9.TAROUNTULA),
    SPIDOPS(Generation9.SPIDOPS),
    NYMBLE(Generation9.NYMBLE),
    LOKIX(Generation9.LOKIX),
    PAWMI(Generation9.PAWMI),
    PAWMO(Generation9.PAWMO),
    PAWMOT(Generation9.PAWMOT),
    TANDEMAUS(Generation9.TANDEMAUS),
    MAUSHOLD(Generation9.MAUSHOLD),
    FIDOUGH(Generation9.FIDOUGH),
    DACHSBUN(Generation9.DACHSBUN),
    SMOLIV(Generation9.SMOLIV),
    DOLLIV(Generation9.DOLLIV),
    ARBOLIVA(Generation9.ARBOLIVA),
    SQUAWKABILLY(Generation9.SQUAWKABILLY),
    NACLI(Generation9.NACLI),
    NACLSTACK(Generation9.NACLSTACK),
    GARGANACL(Generation9.GARGANACL),
    CHARCADET(Generation9.CHARCADET),
    ARMAROUGE(Generation9.ARMAROUGE),
    CERULEDGE(Generation9.CERULEDGE),
    TADBULB(Generation9.TADBULB),
    BELLIBOLT(Generation9.BELLIBOLT),
    WATTREL(Generation9.WATTREL),
    KILOWATTREL(Generation9.KILOWATTREL),
    MASCHIFF(Generation9.MASCHIFF),
    MABOSSTIFF(Generation9.MABOSSTIFF),
    SHROODLE(Generation9.SHROODLE),
    GRAFAIAI(Generation9.GRAFAIAI),
    BRAMBLIN(Generation9.BRAMBLIN),
    BRAMBLEGHAST(Generation9.BRAMBLEGHAST),
    TOEDSCOOL(Generation9.TOEDSCOOL),
    TOEDSCRUEL(Generation9.TOEDSCRUEL),
    KLAWF(Generation9.KLAWF),
    CAPSAKID(Generation9.CAPSAKID),
    SCOVILLAIN(Generation9.SCOVILLAIN),
    RELLOR(Generation9.RELLOR),
    RABSCA(Generation9.RABSCA),
    FLITTLE(Generation9.FLITTLE),
    ESPATHRA(Generation9.ESPATHRA),
    TINKATINK(Generation9.TINKATINK),
    TINKATUFF(Generation9.TINKATUFF),
    TINKATON(Generation9.TINKATON),
    WIGLETT(Generation9.WIGLETT),
    WUGTRIO(Generation9.WUGTRIO),
    BOMBIRDIER(Generation9.BOMBIRDIER),
    FINIZEN(Generation9.FINIZEN),
    PALAFIN(Generation9.PALAFIN),
    PALAFIN_HERO(Generation9.PALAFIN_HERO),
    VAROOM(Generation9.VAROOM),
    REVAVROOM(Generation9.REVAVROOM),
    CYCLIZAR(Generation9.CYCLIZAR),
    ORTHWORM(Generation9.ORTHWORM),
    GLIMMET(Generation9.GLIMMET),
    GLIMMORA(Generation9.GLIMMORA),
    GREAVARD(Generation9.GREAVARD),
    HOUNDSTONE(Generation9.HOUNDSTONE),
    FLAMIGO(Generation9.FLAMIGO),
    CETODDLE(Generation9.CETODDLE),
    CETITAN(Generation9.CETITAN),
    VELUZA(Generation9.VELUZA),
    DONDOZO(Generation9.DONDOZO),
    TATSUGIRI(Generation9.TATSUGIRI),
    ANNIHILAPE(Generation9.ANNIHILAPE),
    CLODSIRE(Generation9.CLODSIRE),
    FARIGIRAF(Generation9.FARIGIRAF),
    DUDUNSPARCE(Generation9.DUDUNSPARCE),
    DUDUNSPARCE_THREE_SEGMENT(Generation9.DUDUNSPARCE_THREE_SEGMENT),
    KINGAMBIT(Generation9.KINGAMBIT),
    GREATTUSK(Generation9.GREATTUSK),
    SCREAMTAIL(Generation9.SCREAMTAIL),
    BRUTEBONNET(Generation9.BRUTEBONNET),
    FLUTTERMANE(Generation9.FLUTTERMANE),
    SLITHERWING(Generation9.SLITHERWING),
    SANDYSHOCKS(Generation9.SANDYSHOCKS),
    IRONTREADS(Generation9.IRONTREADS),
    IRONBUNDLE(Generation9.IRONBUNDLE),
    IRONHANDS(Generation9.IRONHANDS),
    IRONJUGULIS(Generation9.IRONJUGULIS),
    IRONMOTH(Generation9.IRONMOTH),
    IRIDTHORNS(Generation9.IRONTHORNS),
    FRIGIBAX(Generation9.FRIGIBAX),
    ARCTIBAX(Generation9.ARCTIBAX),
    BAXCALIBUR(Generation9.BAXCALIBUR),
    GIMMIGHOUL(Generation9.GIMMIGHOUL),
    GIMMIGHOUL_ROAMING(Generation9.GIMMIGHOUL_ROAMING),
    GHOLDENGO(Generation9.GHOLDENGO),
    WOCHIEN(Generation9.WOCHIEN),
    CHIENPAO(Generation9.CHIENPAO),
    TINGLU(Generation9.TINGLU),
    CHIYU(Generation9.CHIYU),
    ROARINGMOON(Generation9.ROARINGMOON),
    IRONVALIANT(Generation9.IRONVALIANT),
    KORAIDON(Generation9.KORAIDON),
    MIRAIDON(Generation9.MIRAIDON),
    WALKINGWAKE(Generation9.WALKINGWAKE),
    IRONLEAVES(Generation9.IRONLEAVES),
    DIPPLIN(Generation9.DIPPLIN),
    POLTCHAGEIST(Generation9.POLTCHAGEIST),
    SINISTCHA(Generation9.SINISTCHA),
    OKIDOGI(Generation9.OKIDOGI),
    MUNKIDORI(Generation9.MUNKIDORI),
    FEZANDIPITI(Generation9.FEZANDIPITI),
    OGERPON(Generation9.OGERPON),
    OGERPON_WELLSPRING(Generation9.OGERPON_WELLSPRING),
    OGERPON_HEARTHFLAME(Generation9.OGERPON_HEARTHFLAME),
    OGERPON_CORNERSTONE(Generation9.OGERPON_CORNERSTONE),
    ARCHALUDON(Generation9.ARCHALUDON),
    HYDRAPPLE(Generation9.HYDRAPPLE),
    GOUGINGFIRE(Generation9.GOUGINGFIRE),
    RAGINGBOLT(Generation9.RAGINGBOLT),
    IRONBOULDER(Generation9.IRONBOULDER),
    IRONCROWN(Generation9.IRONCROWN),
    TERAPAGOS(Generation9.TERAPAGOS),
    TERAPAGOS_TERASTAL(Generation9.TERAPAGOS_TERASTAL),
    TERAPAGOS_STELLAR(Generation9.TERAPAGOS_STELLAR),
    PECHARUNT(Generation9.PECHARUNT),
    BLOODMOON_URSALUNA(Generation9.BLOODMOON_URSALUNA),
    PALDEAN_TAUROS(Generation9.PALDEAN_TAUROS),
    PALDEAN_TAUROS_BLAZE(Generation9.PALDEAN_TAUROS_BLAZE),
    PALDEAN_TAUROS_AQUA(Generation9.PALDEAN_TAUROS_AQUA),
    PALDEAN_WOOPER(Generation9.PALDEAN_WOOPER);


    Species(Generation1 g) {
        EVOLUTION = null;
        HP = g.getHP();
        NAME = g.getName();
        NAT_DEX_NUMBER = g.getDexNumber();
        TYPE = g.getType();
        IMG = g.getImg();
    }

    Species(Generation2 g) {
        EVOLUTION = null;
        HP = g.getHP();
        NAME = g.getName();
        NAT_DEX_NUMBER = g.getDexNumber();
        TYPE = g.getType();
        IMG = g.getImg();
    }

    Species(Generation3 g) {
        EVOLUTION = null;
        HP = g.getHP();
        NAME = g.getName();
        NAT_DEX_NUMBER = g.getDexNumber();
        TYPE = g.getType();
        IMG = g.getImg();
    }

    Species(Generation4 g) {
        EVOLUTION = null;
        HP = g.getHP();
        NAME = g.getName();
        NAT_DEX_NUMBER = g.getDexNumber();
        TYPE = g.getType();
        IMG = g.getImg();
    }

    Species(Generation5 g) {
        EVOLUTION = null;
        HP = g.getHP();
        NAME = g.getName();
        NAT_DEX_NUMBER = g.getDexNumber();
        TYPE = g.getType();
        IMG = g.getImg();
    }

    Species(Generation6 g) {
        EVOLUTION = null;
        HP = g.getHP();
        NAME = g.getName();
        NAT_DEX_NUMBER = g.getDexNumber();
        TYPE = g.getType();
        IMG = g.getImg();
    }

    Species(Generation7 g) {
        EVOLUTION = null;
        HP = g.getHP();
        NAME = g.getName();
        NAT_DEX_NUMBER = g.getDexNumber();
        TYPE = g.getType();
        IMG = g.getImg();
    }

    Species(Generation8 g) {
        EVOLUTION = null;
        HP = g.getHP();
        NAME = g.getName();
        NAT_DEX_NUMBER = g.getDexNumber();
        TYPE = g.getType();
        IMG = g.getImg();
    }

    Species(Generation9 g) {
        EVOLUTION = null;
        HP = g.getHP();
        NAME = g.getName();
        NAT_DEX_NUMBER = g.getDexNumber();
        TYPE = g.getType();
        IMG = g.getImg();
    }

    private final Species[] EVOLUTION;
    private final int HP;
    private final Type[] TYPE;
    private final String NAME, NAT_DEX_NUMBER;
    private ImageIcon IMG;
    public boolean hasEvolutions() {
        return EVOLUTION != null;
    }

    public String getDexNumber()
    {
        return NAT_DEX_NUMBER;
    }


    public String getName()
    {
        return NAME;
    }

    public int getHP(){
        return HP;
    }

    public Type[] getType()
    {
        return TYPE;
    }

    public ImageIcon getImg(){
        return IMG;
    }
}
